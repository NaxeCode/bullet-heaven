...
void Transform_obj::startAction(int whichMarker){
            	...
            	HX_STACK_THIS(this)
            	HX_STACK_ARG(whichMarker,"whichMarker")
HXLINE( 114)
...
HXLINE( 154)		HX_VARI( ::String,cursorName) = HX_("",00,00,00,00);
HXLINE( 156)		{
HXLINE( 156)			int _g = 0;
HXDLIN( 156)			int _g1 = this->_markers->length;
HXDLIN( 156)			while((_g < _g1)){
HXLINE( 156)				_g = (_g + 1);
HXDLIN( 156)				HX_VARI( int,i) = (_g - 1);
...
HXLINE( 176)
HXLINE( 177)		HX_VARI( Float,value) = ::flixel::math::FlxMath_obj::roundDecimal(number,::flixel::FlxG_obj::debugger->precision);
...
void Transform_obj::updateScaleActionDirection(){
            	...
            	HX_STACK_THIS(this)
HXLINE( 192)
HXLINE( 193)		HX_VARI( Float,deltaX) = (this->_mouseCursor->x - this->_actionStartPoint->x);
HXLINE( 194)		HX_VARI( Float,deltaY) = (this->_mouseCursor->y - this->_actionStartPoint->y);
HXLINE( 196)		{
...
HXLINE( 206)		HX_VARI( Float,deltaX) = ((this->_actionScaleDirection->x * ::Math_obj::abs((this->_mouseCursor->x - this->_actionStartPoint->x))) / ((Float)10.0));
HXLINE( 207)		HX_VARI( Float,deltaY) = ((this->_actionScaleDirection->y * ::Math_obj::abs((this->_mouseCursor->y - this->_actionStartPoint->y))) / ((Float)10.0));
...
void Transform_obj::updateMarkersPosition(){
            	...
            	HX_STACK_THIS(this)
HXLINE( 238)
HXLINE( 239)		HX_VARI( Float,topLeftX) = (this->_targetArea->x - ((Float)5.0));
HXLINE( 240)		HX_VARI( Float,topLeftY) = (this->_targetArea->y - ((Float)5.0));
HXLINE( 241)		HX_VARI( Float,width) = (this->_targetArea->width + ((Float)10.));
HXLINE( 242)		HX_VARI( Float,height) = (this->_targetArea->height + ((Float)10.));
HXLINE( 244)		{
...
void Transform_obj::updateMarkersRotation(Float outlineWidth,Float outlineHeight){
            	...
            	HX_STACK_ARG(outlineHeight,"outlineHeight")
HXLINE( 254)
HXLINE( 255)		HX_VARI( Float,rotationAngleRad) = (this->_target->angle * (::Math_obj::PI / ( (Float)(180) )));
HXLINE( 256)		HX_VARI( Float,originX) = (this->_markers->__get(0).StaticCast<  ::flixel::math::FlxBasePoint >()->x + (outlineWidth / ( (Float)(2) )));
HXLINE( 257)		HX_VARI( Float,originY) = (this->_markers->__get(0).StaticCast<  ::flixel::math::FlxBasePoint >()->y + (outlineHeight / ( (Float)(2) )));
...
HXLINE( 261)				HX_VARI(  ::flixel::math::FlxBasePoint,marker) = _g1->__get(_g).StaticCast<  ::flixel::math::FlxBasePoint >();
HXDLIN( 261)				_g = (_g + 1);
HXLINE( 264)				HX_VARI( Float,rotatedX) = (((marker->x - originX) * cos) - ((marker->y - originY) * sin));
HXLINE( 265)				HX_VARI( Float,rotatedY) = (((marker->x - originX) * sin) + ((marker->y - originY) * cos));
HXLINE( 266)				{
...
void Transform_obj::draw(){
            	...
            	HX_STACK_THIS(this)
HXLINE( 317)
HXLINE( 318)		HX_VARI(  ::openfl::display::Graphics,gfx) = this->_brain->getDebugGraphics();
...
HXDLIN( 338)			HX_VARI(  ::flixel::group::FlxTypedGroupIterator,member) =  ::flixel::group::FlxTypedGroupIterator_obj::__alloc( HX_CTX ,this->_brain->selectedItems->members,filter);
HXDLIN( 338)			while(member->hasNext()){
HXLINE( 338)				HX_VARI_NAME(  ::flixel::FlxObject,member1,"member") = member->next().StaticCast<  ::flixel::FlxObject >();
